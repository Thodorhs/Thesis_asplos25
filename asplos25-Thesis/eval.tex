...
\note{jk: avoid to use many parenthesis in the text} latency is 4.22× higher
than local with an average of 59.86 usec. By
looking at the Exported ramdisk with NBD (TCP) average latency \note{jk: do not
use very close - write to the point} is 1.08$\times$ better now, meaning that
...
drivers and we get 8.31 usec average latency 1.7 × better \note{better compared
to what ??}. We also try to lower
...
\note{jk: fix the next sentences. You cannot have a paragraph with one sentence}
